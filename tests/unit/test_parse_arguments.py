...
def test_minimal_argument():
    ...
    assert(args == ('test.bed', 'test.gtf', 50, 1, 5,
        'test_rollmean50_stdev1_minGeneCount5.bed', None, 10, 5))

def test_optional_argument():
    args = clip.parse_arguments(['-i', 'test.bed', '-a', 'test.gtf', '-o', 'test', '-n', '100'])
    assert(isinstance(args, tuple))
    assert(args == ('test.bed', 'test.gtf', 100, 1, 5,
        'test_rollmean100_stdev1_minGeneCount5.bed', None, 10, 5))
