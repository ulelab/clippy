...
genome_filepath = "tests/data/genome.fa.fai"

annot = pybedtools.BedTool("tests/data/annot.gff")

annot_bed = annot.filter(lambda row: row.fields[2] == "gene").saveas()

one_base_flanks = annot_bed.flank(g=genome_filepath, l=1, r=1, s=True)

non_overlapping_flanks = one_base_flanks.intersect(annot_bed, v=True, s=True)

full_length_flanks = annot_bed.flank(g=genome_filepath, l=100, r=100, s=True)
...
valid_flanks = (
    split_flanks.intersect(non_overlapping_flanks, wo=True, s=True)
